...
# Define parameters for alignments (note that only marine values are used in Mehinto et al. (2022)) #
...
#### Alpha value assignment for ERM transformations below ####
...
mux.polyfnx_generalizable = function(a.x, x_UL, x_LL) {
...
aoc_final_small <- aoc_intermediate_alphas  %>% 
 ...
 # calculate effect threshold for particles
  ...
  mutate(mu.p.poly = mux.polyfnx_generalizable(a.x = alpha, #alpha for particles
...
##--- environmental calculations ---###
  ...
  mutate(mu.sa.poly = mux.polyfnx_generalizable(a.sa, x_UL_sa, x_LL_sa)) %>% 
  ...
  mutate(mu.sa.mono = case_when(
    ...
    polydispersity == "polydisperse" ~  mux.polyfnx_generalizable(a.x = a.sa, 
...
##--- environmental calculations ---###
  ...
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx_generalizable(a.v, x_UL_v, x_LL_v)) %>% 
  ...
  mutate(mu.v.mono = case_when(
    ...
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.v, 
...
##--- environmental calculations ---###
  ...
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               ...
                               p = p.ave)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx_generalizable(a.m, x_UL_m, x_LL_m)) %>% 
  ...
  mutate(mu.m.mono = case_when(
    ...
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.m, 
...
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  ...
  mutate(mu.ssa.inverse.poly = mux.polyfnx_generalizable(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
...
preds <- predict(dists,
                 average = TRUE, 
                 #ic = "aicc", #functionality removed from SSDs package :(
                 nboot = 10, ci= TRUE) 
...
filtered.data.large.base_t1.2 <- aoc_final_large %>% 
         ...
         drop_na(dose_new) %>% 
         filter(dose_new > 0) %>%  #remove zero values
...
## Summary Table
 Note that confidence intervals differ slightly from those in Mehinto et al. (2022) due to the inability to use aicc as a model averaging criteria in the ssdtools() package.
...
#### Prediction ####
set.seed(99)
pred_tier3_4_large_median <- predict(dists_tier3_4_large_median, average = TRUE, #ic = "aicc",
                                     nboot = 10, ci= TRUE) 
...
#### Prediction ####
set.seed(99)
pred_tier3_4_small_median <- predict(dists_tier3_4_small_median, average = TRUE, #ic = "aicc",
                                     nboot = 10, ci= TRUE) 
...
ggsave(filename = "body_size_figs_SIx.jpeg",
       path = "Threshold_Manuscript_Figs/", 
...
#write.xlsx(list_of_datasets, file = "Threshold_Manuscript_Figs/SSD_rawdata_102121.xlsx")
...
oxidative.stress.tiers <- ERM_small %>% 
  mutate(EC_env_sa_um2.L =  `Surface Area` * mux.polyfnx_generalizable(a.x = 1.50, #alpha for marine surface water for surface area
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  mutate(EC_env_mass_ug.L.SA =  `Surface Area` * mux.polyfnx_generalizable(a.x = 1.32, #alpha for marine surface water for mass
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
...
food.dilution.tiers <- ERM_large %>% 
  mutate(EC_env_volume_um3.L =  Volume * mux.polyfnx_generalizable(a.x = 1.48, #alpha for marine surface water for surface area
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  mutate(EC_env_mass_ug.L.V =  Volume * mux.polyfnx_generalizable(a.x = 1.32, #alpha for marine surface water for mass
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
...
gtsave(marine_thresholds_table, 
       filename = "Threshold_Manuscript_Figs/marine_thresholds_table.html")

#save as csv
write.csv(simple.summary,
          "Threshold_Manuscript_Figs/marine_thresholds_table.csv")
...
food.dilution.tiers_freshwater <- ERM_large_freshwater %>% 
 # drop_na(Volume) %>% 
  mutate(EC_env_volume_um3.L =  Volume * mux.polyfnx_generalizable(a.x = a.v.freshwater, #alpha for surface area
                                       x_UL = 5000, #upper limit of default range
                                       x_LL = 1 # lower limit of default range
                                       )) %>% 
  mutate(EC_env_mass_ug.L.V =  Volume * mux.polyfnx_generalizable(a.x = a.m.freshwater, #alpha for  mass
                                       x_UL = 5000, #upper limit of default range
                                       x_LL = 1 # lower limit of default range
                                       )) %>%
  dplyr::select(EC_env_volume_um3.L, EC_env_mass_ug.L.V, Volume)
...
write.csv(endpoints_sensitivity_thresholds,
          "Threshold_Manuscript_Figs/endpoints_sensitivity.csv")
...
write.csv(biological_level_sensitivity_thresholds,
          "Threshold_Manuscript_Figs/biological_level_sensitivity.csv")
...
write.csv(bioaccessibility_sensitivity_thresholds,
          "Threshold_Manuscript_Figs/bioaccessibility_sensitivity.csv")
...
##--- environmental calculations ---###
  ...
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx_generalizable(a.x = a.sa.lower,  #use alpha = 1 eq.
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.v.lower, #use alpha = 1 eq.
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.m.lower, #use alpha = 1 eq.
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
...
##--- environmental calculations ---###
 ...
 #calculate mu_x_poly (env) for surface area
  ...
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx_generalizable(a.x = a.sa.lower, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
...
##--- environmental calculations ---###
  ...
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.v.lower, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
 ...
 # calculate mu.m.poly
  ...
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.m.lower, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
...
##--- environmental calculations ---###
  ...
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx_generalizable(a.x = a.sa.upper, #use alpha = 2 eq.
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.v.upper, #use alpha = 2 eq.
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.m.upper, #use alpha = 2 eq.
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
...
##--- environmental calculations ---###
 ...
 #calculate mu_x_poly (env) for surface area
  ...
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx_generalizable(a.x = a.sa.upper, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
...
##--- environmental calculations ---###
  ...
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.v.upper, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
 ...
 # calculate mu.m.poly
  ...
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.m.upper, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
...
write.csv(polydispersity_sensitivity_thresholds,
          "Threshold_Manuscript_Figs/polydispersity_sensitivity.csv")
...
aoc_final_small_lower_alignment <- aoc_intermediate_alphas  %>% 
 ...
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx_generalizable(a.x = alpha.lower, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
...
##--- environmental calculations ---###
  ...
  #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx_generalizable(a.sa.lower, x_UL_sa, x_LL_sa)) %>% 
  ...
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx_generalizable(a.x = a.sa,  #use alpha = 1 eq.
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx_generalizable(a.v.lower, x_UL_v, x_LL_v)) %>% 
  ...
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.v, #use alpha = 1 eq.
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               ...
                               p = p.ave.lower)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx_generalizable(a.m.lower, x_UL_m, x_LL_m)) %>% 
  ...
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx_generalizable(a.x = a.m, #use alpha = 1 eq.
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
...
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  ...
  #calculate mu_x_poly for specific surface area
  mutate(mu.ssa.inverse.poly = mux.polyfnx_generalizable(a.ssa.lower, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
...
aoc_final_small_upper_alignment <- aoc_intermediate_alphas  %>% 
 ...
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx_generalizable(a.x = alpha.upper, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
...
##--- environmental calculations ---###
  ...
  #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx_generalizable(a.sa.upper, x_UL_sa, x_LL_sa)) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx_generalizable(a.v.upper, x_UL_v, x_LL_v)) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               ...
                               p = p.ave.upper)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx_generalizable(a.m.upper, x_UL_m, x_LL_m)) %>% 
...
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  ...
  #calculate mu_x_poly for specific surface area
  mutate(mu.ssa.inverse.poly = mux.polyfnx_generalizable(a.ssa.upper, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
...
write.csv(alignment_sensitivity_thresholds,
          "Threshold_Manuscript_Figs/alignment_sensitivity.csv")
