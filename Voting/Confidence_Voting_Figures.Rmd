...
```{r}
getwd()
votes_rounds1.2 <- read_excel("voting_ambient.xlsx", sheet  ="Tidy") %>% 
  mutate(Value = as.numeric(Value)) %>% 
  mutate_if(is.character,as.factor)

#pivot data to get in tidy format compatible wiht other dataframe
votes_round3 <- read_excel("voting_ambient.xlsx", sheet  ="Round 3_tidy",
                           na = "NA") %>% 
...
final <- cast %>% 
  group_by(Person, Threshold, Round, ERM) %>%
    dplyr::summarise(Confidence = sum(Confidence, na.rm = T),
